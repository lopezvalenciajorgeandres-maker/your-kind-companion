...
export const exportFullBackup = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const sheets: BackupSheets = {
      ...
      [SHEETS.appointments]: appointments.map((a) => ({ id: a.id, inicio_iso: a.starts_at, fin_iso: a.ends_at, cliente_id: a.client_id, cliente: clientName.get(a.client_id) ?? "", servicio_id: a.service_id ?? "", servicio: serviceName.get(a.service_id) ?? "", profesional_id: a.professional_id ?? "", profesional: professionalName.get(a.professional_id) ?? "", tratamiento_id: a.treatment_id ?? "", estado: a.status, origen: a.origin, valor_centavos: a.price_cents ?? "", valor: (a.price_cents ?? 0) / 100, notas: a.notes ?? "", firma_datos: a.signature_data_url ?? "", firmado_iso: a.signed_at ?? "", firmado_por: a.signed_by_name ?? "", creado_iso: a.created_at })),
...
export const importFullBackup = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await merge("package_sessions", get(SHEETS.packageSessions), "sesiones_paquete", (r) => { const package_id = resolve(r, ["paquete_id"], [], packageMap, packagesNow, (x) => x.name); const client_id = resolve(r, ["cliente_id"], ["cliente"], clientMap, clientsNow, (x) => [x.full_name, x.last_name ?? ""].join(" ").trim()); const oldAppt = pick(r, ["cita_id"]); const appointment_id = oldAppt ? apptMap.get(oldAppt) ?? apptsNow.find((x) => x.id === oldAppt)?.id ?? null : null; return package_id && client_id ? { package_id, client_id, appointment_id, used_at: iso(pick(r, ["usado_iso", "used_at"])), expires_at: iso(pick(r, ["vence_iso", "expires_at"])) } : null; }, (r, xs) => { const pid = resolve(r, ["paquete_id"], [], packageMap, packagesNow, (x) => x.name); const cid = resolve(r, ["cliente_id"], [], clientMap, clientsNow, (x) => x.full_name); return xs.find((x) => x.package_id === pid && x.client_id === cid && String(x.used_at ?? "") === String(iso(pick(r, ["usado_iso", "used_at"])) ?? "")); });

    await merge("payments", get(SHEETS.payments), "pagos", (r) => { const paid_at = iso(pick(r, ["fecha_iso", "fecha", "paid_at"])); if (!paid_at) return null; const oldTreatment = pick(r, ["tratamiento_id"]); const oldAppt = pick(r, ["cita_id"]); const treatment_id = oldTreatment ? treatmentMap.get(oldTreatment) ?? treatmentsNow.find((x) => x.id === oldTreatment)?.id ?? null : null; const appointment_id = oldAppt ? apptMap.get(oldAppt) ?? apptsNow.find((x) => x.id === oldAppt)?.id ?? null : null; if ((oldTreatment && !treatment_id) || (oldAppt && !appointment_id)) { warnings.push("Pago omitido: no se pudo relacionar con su cita o tratamiento"); return null; } return { client_id: resolve(r, ["cliente_id"], ["cliente"], clientMap, clientsNow, (x) => [x.full_name, x.last_name ?? ""].join(" ").trim()), service_id: resolve(r, ["servicio_id"], ["servicio"], serviceMap, servicesNow, (x) => x.name), treatment_id, appointment_id, amount_cents: cents(r, ["abono_centavos", "amount_cents"], ["abono", "importe", "monto", "amount"]), total_cents: pick(r, ["total_centavos"]) ? Math.round(num(pick(r, ["total_centavos"]))) : (pick(r, ["total"]) ? Math.round(num(pick(r, ["total"])) * 100) : null), method: pick(r, ["metodo", "method"]) || "efectivo", bank: pick(r, ["banco", "bank"]) || null, status: pick(r, ["estado", "status"]) || "pagado", paid_at, notes: pick(r, ["notas", "notes"]) || null }; }, (r, xs) => xs.find((x) => x.client_id === resolve(r, ["cliente_id"], ["cliente"], clientMap, clientsNow, (y) => [y.full_name, y.last_name ?? ""].join(" ").trim()) && x.paid_at === iso(pick(r, ["fecha_iso", "fecha", "paid_at"])) && x.amount_cents === cents(r, ["abono_centavos", "amount_cents"], ["abono", "importe", "monto", "amount"])));
